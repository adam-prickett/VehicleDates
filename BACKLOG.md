...
- [ ] **Notifications — further providers (Twilio, Slack, Discord, email, generic webhook).** ntfy and Pushover are now shipped. Each addition is a single file under `src/server/notifications/providers/` plus a line in `providers/index.ts`, and a per-provider field block in the channel modal.
