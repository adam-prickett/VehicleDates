...
- [ ] **Notifications — second provider (Pushover, Twilio, etc.).** Only ntfy is implemented. Each addition is a single file under `src/server/notifications/providers/` plus a line in `providers/index.ts`. The channel modal also needs provider-aware fields; today it switches on `type === "ntfy"`.
